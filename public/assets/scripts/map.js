...
function drawData (kind) {
  ...
  $.getJSON(`api/${kindUrl}`, function (data) {
    ...
    Object.keys(markerLayers).forEach((layer, a) => {
...
function generateLegendTable (players) {
  ...
  Object.keys(guilds).forEach(function (name) {
    tableContent += `
      <tr class="legend-list-item ${(disabledFilters[guilds[name].id]) ? 'disabled' : ''}" data-legend="${guilds[name].id}" onClick="toggleLegend('${guilds[name].id}')">
        <td style="background-color: ${guilds[name].color}"></td>
        <td>${guilds[name].id}</td>
        <td>${name}</td>
      </tr>
    `
  })
  ...
  Object.keys(solos).forEach(function (name) {
    tableContent += `
      <tr class="legend-list-item ${(disabledFilters[solos[name].id]) ? 'disabled' : ''}" data-legend="${solos[name].id}" onClick="toggleLegend('${solos[name].id}')">
